...
class StoremasterController extends Controller
{
    ...
    public function update(UpdateRequest $request, Storemasters $storemasters)
    {	
        if ($store = $request->persist()->getStoremasters()) {
            flashWebResponse('updated', 'store master');
            return redirect()->route('store-master-edit', $store->id);
        }
        flashWebResponse('error');
        return redirect()->back();
    }

...
class NewsfeedController extends Controller
{
    ...
    public function store(StoreRequest $request)
    {
    	if($level = $request->persist()->getPost()){
            ...
            return redirect()->route('newsfeed', $level->id);
    	}
    ...
    public function newsfeed_comment_likes(Request $request){
        ...
        if($get_data->count() >= 1){ 
            $like = Newsfeedcommentlike::where($data)->delete();
            $is_like = false;
        }else{ 
    ...
    public function deleteNewsfeed(DeleteNewsfeedRequest $request, User $user) {
        ...
        if (request()->ajax()) {
            ...
            return response()->json(['status' => 'success', 'title' => 'Created!', 'text' => 'You have succussfully delete post.!', 'deleted_id' => $data['deleted_id']]);
        }
        return errorWebResponse();
    }
    ...
    public function loadMore(Request $request){ 
        if($request->ajax()){
            ...
            $results = Newsfeed::select('*')->orderBy('newsfeeds.id','desc')->skip($skip)->take($take)->get();
            $userinfo = Userinfo::where('user_id',Auth::user()->id)->first();
            $friends_id = Friendlist::select('friend_id')->where('user_id',Auth::user()->id)->pluck('friend_id');
            ...
            foreach($results as $result){
                ...
                $profile_img = $system_path = url('public/images/profile/'.$result['userImageByPost']['profile_image']);
                $profile_default_img = url('public/assets/dashboard/img/noimage.jpg');
                ...
                foreach($result['NewsfeedGallaries'] as $imageValue){
                    $output .='<img loading="lazy" src="'.url('public/images/newsfeed/'.$imageValue['image']).'" alt="photo" width="488" height="194" ><br>';
                }
                $output .= '</div>
            ...
            $output .= '<img loading="lazy" src="'. url('public/images/profile',$data['userinfo']['profile_image'] ).'" width="36" height="36" alt="author" class="rounded-circle">';
                    }
                ...
                <li class="comment-item reply_comment_add_'.$comment->id.'">
                    <div class="post__author author vcard inline-items">';
                        if(isset($comment->profileImage->profile_image)){
                            $output .= '<img loading="lazy" src="'. url('public/images/profile',$comment->profileImage->profile_image) .'" width="40" height="40" alt="author">';
                        }else{
                            $output .= '<img loading="lazy" src="'. url('public/assets/dashboard/img/noimage.jpg') .'" width="36" height="36" alt="author">';
                        }
        ...
        $output .=  '<a href="javascript:void(0)" class="reply comment_reply_btn" id="'. $comment->id.'">Reply</a>
                    ...
                    <!-- Reply Comment Form  -->  
                        ...
                        <form class="inline-items comment-reply-form comment_reply_add_'.$comment->id.'" route="'. route('comment_reply_add').'" user_id="'. Auth::user()->id .'" newsfeed_id="'. $result->id .'" comment_id="'.$comment->id.'" id="">
                            ...
                            $output .='<img loading="lazy" src="'. url('public/images/profile',$userinfo->profile_image ) .'" width="36" height="36" alt="author" class="rounded-circle">';
                                }
                    ...
                    <li class="comment-item">
                        <div class="post__author author vcard inline-items">';
                            ...
                            $output .='<img loading="lazy" src="'. url('public/images/profile',$replyComment->profileImage->profile_image) .'" width="40" height="40" alt="author">';
                            }else{
                            $output .='<img loading="lazy" src="'. url('public/assets/dashboard/img/noimage.jpg') .'" width="36" height="36" alt="author">';
                            }
                            $output .='<div class="author-date">

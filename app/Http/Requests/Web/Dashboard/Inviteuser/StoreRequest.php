...
class StoreRequest extends FormRequest
{
    ...
    public function persist(): self
    {
        ...
        foreach ($emails as $value) {
           $data[] = ['user_id' => Auth::user()->id, 'email' => $value, 'message' => trim($message)];
           SendEmailJob::dispatch(['email' => $value,'message' => trim($message), 'name' => Auth::user()->name]);
        }

        InviteUser::insert($data);
        return $this;

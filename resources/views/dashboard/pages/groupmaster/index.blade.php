		...
		<div class="col col-xl-12 order-xl-2 col-lg-12 order-lg-2 col-md-12 order-md-1 col-sm-12 col-12">
			...
			<div class="ui-block">
				<div class="ui-block-title">
					...
					<a href="{{ route('group-create') }}" class="btn btn-primary btn-md-2 float-right">Add New Group<div class="ripple-container"></div></a>
	                    ...
	                    @foreach ($results as $index => $result)
							<tr>
								...
								@if(!empty($result->image))
									<img src="{{ url('public/'.$result->image) }}"  style="max-width: 40px;">
								@endif
								</td>
								<td class="store">
									@foreach($result->groupAdmin as $user)
		                                <span class="badge bg-primary">{{ @$user->userData->name }}</span>
									@endforeach
								</td>
								<td class="posts">
									@if($result->status == 1)							
								...
								<td class="freshness">
									<a href="{{ route('group-edit',$result->id) }}" class="btn btn-sm bg-blue">Edit</a>
									<a href="javascript:void(0)" route="{{ route('group-delete',$result->id) }}" id="{{$result->id}}" text="remove level" class="btn btn-sm btn-border-think btn-transparent c-grey delete">Delete<div class="ripple-container"></div></a>
								</td>
							</tr>
						@endforeach
	                    @else
